    ...
    if request.method == 'POST':
        try:
            ...
            if passport_file and passport_file.filename.strip():
                ...
                current_app.logger.info(f"[PASSPORT UPLOADED] {passport_url}")
            ...
            current_app.logger.info(f"[WORKER CREATED] {worker_code} - {name}")
            ...
            db.session.rollback()
            current_app.logger.error(f"REGISTER WORKER ERROR: {traceback.format_exc()}")
...
def send_worker_letter(worker_id):
    ...
    try:
        # =========================
        # FIX: ensure HTML is properly rendered
        # =========================
        html_body = render_template(
            'emails/worker_letter_email.html',
            worker=worker,
            company_logo=current_app.config.get("COMPANY_LOGO_URL")
        )

        if not html_body or html_body.strip() == "":
            raise ValueError("Rendered email HTML is empty")

        current_app.logger.info(f"EMAIL HTML LENGTH: {len(html_body)}")
        ...
        response = requests.post(
            ...
            json={
                "sender": {
                    "name": "Okoya Food Company Limited",
                ...
                "subject": f"Official HR Letter - {worker.name}",
        ...
        current_app.logger.info(
            f"BREVO RESPONSE: {response.status_code} - {response.text}"
        )
...
@workers_bp.route('/debug-mail-config')
@login_required(role='admin')
...
def edit_worker(worker_id):
    ...
    if request.method == 'POST':
        try:
                ...
                if allowed_file(passport_file.filename):
                    ...
                    if worker.passport and 'cloudinary.com' in worker.passport:
                        try:
                            ...
                            current_app.logger.warning(f"Failed to delete old passport: {e}")
            ...
            db.session.rollback()
            current_app.logger.error(f"EDIT WORKER ERROR: {traceback.format_exc()}")
    ...
    try:
        if worker.passport and 'cloudinary.com' in worker.passport:
            ...
            try:
                ...
                current_app.logger.warning(f"Failed to delete passport: {e}")
        ...
        db.session.rollback()
        current_app.logger.error(f"DELETE WORKER ERROR: {traceback.format_exc()}")
